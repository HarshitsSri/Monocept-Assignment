...
public class CourseDAO {
    ...
    public boolean deleteIfNoRegistration(int courseId) {

        if (hasRegistration(courseId)) {
            return false;
        }
    ...
    public boolean hasRegistration(int courseId) {

        String sql =
                "SELECT 1 FROM registrations WHERE course_id = ? LIMIT 1";
        ...
        try (Connection con = DBConnection.getConnection();
            ...
            throw new RuntimeException(
                    "Error while checking course registrations", e);
        }
    }
